...
class ArtConverter:
    def __init__(self, path='img/Wave.jpg', font_size=12, color_lvl=8):
        pg.init()
        self.path = path
    ...
    def draw_converted_image(self):
        char_indicies = self.gray_image // self.ASCII_COEFF
        color_indicies = self.image // self.COLOR_COEFF
        for x in range(0, self.WIDTH, self.CHAR_STEP):
            for y in range(0, self.HEIGHT, self.CHAR_STEP):
                char_index = char_indicies[x, y]
                if char_index:
                    char = self.ASCII_CHARS[char_index]
                    color = tuple(color_indicies[x, y])
    ...
    def save_image(self):
        ...
        cv2.imwrite('output/ascii_color_image.jpg', cv2_img)
